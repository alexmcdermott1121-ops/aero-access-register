...
create table if not exists public.access_register (
  ...
  access_type text not null check (access_type in ('Key', 'Fob', 'Remote', 'Contractor Access', 'Inspection Access', 'Digital Access', 'Other')),
  access_area text not null check (access_area in ('Common areas', 'Basement', 'Garage', 'Plant room', 'Roof', 'Fire services area', 'Unit access by consent', 'Storage/common area', 'Other')),
  ...
  returned_date date,
  status text not null default 'Pending approval' check (status in ('Pending approval', 'Approved', 'Active', 'Expiring soon', 'Overdue return', 'Returned', 'Revoked', 'Lost', 'Archived')),
  conditions text,
  notes text,
  attachment_url text,
...
create table if not exists public.access_audit_log (
  id uuid primary key default gen_random_uuid(),
  access_record_id uuid not null references public.access_register(id) on delete cascade,
